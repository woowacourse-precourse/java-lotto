...
class LottoRefereeTest {
    ...
    @Mock
    private Buyer buyer;
    private LottoRefer lottoRefer;

    @BeforeEach
    void initReferee() {
        lottoRefer = new LottoRefer();
    ...
    @Test
    @DisplayName("등수 결과 분석이 제대로 되는지 확인")
    ...
    @Test
    @DisplayName("총 상금이 제대로 계산되는지 확인")
    void checkCalculatePrizes() {
        //given
        lottoRefer.analyzeResult(lotteryMachine, buyer);
        assertThat(lottoRefer.sumPrizes()).isEqualTo(2000000000 + 30000000 + 1500000 + 50000 + 5000);
    }
}
