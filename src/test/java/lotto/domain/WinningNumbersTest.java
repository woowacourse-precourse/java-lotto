...
class WinningNumbersTest {
    @DisplayName("당첨 번호의 개수가 6개보다 적다면 예외가 발생한다.")
    @Test
    ...
    @Test
    void createWinningNumbersByOutOfRangeNumber() {
        assertThatThrownBy(() -> new WinningNumbers(List.of(1, 2, 3, 4, 5, 46), 8))
                .isInstanceOf(IllegalArgumentException.class);
        assertThatThrownBy(() -> new WinningNumbers(List.of(1, 2, 3, 4, 5, 0), 8))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @DisplayName("보너스 번호에 중복된 숫자가 있으면 예외가 발생한다.")
    @Test
    void createWinningNumbersBonusByDuplicatedNumber() {
    ...
    @Test
    void createWinningNumberBonusByOutOfRangeNumber() {
        assertThatThrownBy(() -> new WinningNumbers(List.of(1, 2, 3, 4, 5, 45), 46))
                .isInstanceOf(IllegalArgumentException.class);
        assertThatThrownBy(() -> new WinningNumbers(List.of(1, 2, 3, 4, 5, 45), 0))
                .isInstanceOf(IllegalArgumentException.class);
    }
}