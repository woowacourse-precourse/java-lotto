...
public class InputUtils {
    ...
    private void isLottoNumberStyle(String input){
        ...
        if(!Pattern.matches(regex, input)){
            throw new IllegalArgumentException(ErrorMessage.NOT_WINNING_LOTTO_INPUT_STYLE_ERROR.getMessage());
        }
    }
    private void isContainWinningLotto(int bonus, Lotto lottoNumbers) {
        if (lottoNumbers.getNumbers().contains(bonus)){
            throw new IllegalArgumentException(ErrorMessage.NOT_BONUS_SAME_WINNING_LOTTO_ERROR.getMessage());
        }
    }
