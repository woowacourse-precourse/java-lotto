...
public class WinningNumbers {
    ...
    private void countValidate(String[] split) {
        ...
        if (split.length != 6) throw new IllegalArgumentException("숫자 갯수가 6개가 아닙니다.");
    }

    private void rangeValidate(String[] split) {
        // 3-3. 쉼표를 기준으로 나눈 숫자가 숫자 범위 1~45 사이에 없으면 예외처리를 한다.
        for (String s : split) {
            if (!(1<= Integer.parseInt(s) && Integer.parseInt(s) <= 45))
                throw new IllegalArgumentException("숫자가 1과 45 사이에 있지 않습니다.");
        }
    }
