...
public class WinningLotto {
    ...
    public Map<Rank, Integer> getUserLottoScore(List<Lotto> userLottos){
        ...
        userLottos.forEach(userLotto -> {
            Rank rank = Rank.of(winningLottoNumbers.getMatchedCount(userLotto), isBonus(userLotto));
            userLottoScore.put(rank, userLottoScore.get(rank) + 1);
        });
        userLottos.remove(Rank.NOTHING);
        return userLottoScore;
    }

    private boolean isBonus(Lotto userLotto){
        if(userLotto.isContain(bonusNumber)){
            return true;
        }
        return false;
    }
