...
public class LottoWin {
    ...
    private void validateLuckyNumber(List<Integer> luckyNumbers) {
        validateNumberSize(luckyNumbers, 6);
        validateNumberOverlap(luckyNumbers);
    ...
    // 당첨 번호 중복 확인 및 예외 처리
    private void validateNumberOverlap(List<Integer> numbers){
    ...
    private void validateNumber(int number) {
        if (number < 1 && number > 45) {
            throw new IllegalArgumentException("[ERROR] 1~45의 숫자를 입력해주세요");
        }
    }
