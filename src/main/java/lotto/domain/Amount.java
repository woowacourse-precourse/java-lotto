...
public class Amount {
    ...
    public Amount(int value) {
        validateAmountZero(value);
        validateCanDivideIntoOneThousand(value);
        this.value = value;
    }

    private void validateCanDivideIntoOneThousand(int value) {
        if (value % LOTTO_PRICE == REMAINDER_VALUE) {
            return;
        }
    ...
    private void validateAmountZero(int value) {
        if (value == NO_AMOUNT) {
            System.out.println(ERROR_CANT_DIVIDE_INTO_1000_MESSAGE);
            throw new IllegalArgumentException(ERROR_CANT_DIVIDE_INTO_1000_MESSAGE);
        }
