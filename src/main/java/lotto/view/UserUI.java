...
public class UserUI {
    ...
    public void printWinningResult(List<String> messages, List<Integer> allOfTimes, double earningRate) {
        System.out.println(resultMessage);
        for(int idx = 0; idx<messages.size(); idx++){
            String winningMessage = messages.get(idx);
            int len = winningMessage.length();
            winningMessage = winningMessage.substring(0, len-1) + allOfTimes.get(idx) + winningMessage.substring(len-1);
            System.out.println(winningMessage);
        }

        System.out.println(earningsMessage[0] + earningRate + earningsMessage[1]);
    }
}
