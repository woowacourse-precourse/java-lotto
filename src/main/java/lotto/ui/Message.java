package lotto.ui;

public class Message {
    public static final String BUY_LOTTO = "개를 구매했습니다.";
    public static final String INPUT_WINNING = "당첨 번호를 입력해 주세요";
    public static final String INPUT_BONUS = "보너스 번호를 입력해 주세요.";
    public static final String OUTPUT_WINNING = "당첨 통계";
    public static final String MONEY_ERROR = "[ERROR] 로또 구입은 1,000원 단위만 가능합니다.";
    public static final String LOTTO_SIZE_ERROR = "[ERROR]  로또 번호는 6개입니다.";
    public static final String LOTTO_OVERLAP_ERROR = "[ERROR] 로또 번호는 중복될 수 없습니다.";
    public static final String WINNING_RANGE_ERROR = "[ERROR] 당첨 번호의 범위는 1~45입니다.";
    public static final String WINNING_OVERLAP_ERROR = "[ERROR] 당첨 번호는 중복될 수 없습니다.";
    public static final String WINNING_SIZE_ERROR = "[ERROR] 당첨 번호는 6개입니다.";
    public static final String BONUS_NUMBER_ERROR = "[ERROR] 보너스 번호와 당첨 번호는 중복될 수 없습니다.";
}
