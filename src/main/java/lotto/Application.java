...
public class Application {
    ...
    static double getEarningsRate(int sheets, int totalWinningPrice) {
        double earningsRate = 0.00;

        earningsRate = Double.valueOf(totalWinningPrice) / Double.valueOf(sheets * 1000) * Double.valueOf(100);
...
// X 6.프로그램 구현이 완료되면 ApplicationTest의 모든 테스트가 성공 확인
...
// X 11.JUnit 5와 AssertJ를 이용하여 본인이 정리한 기능 목록이 정상 동작함을 테스트 코드로 확인
...
// X 7.단위 테스트 작성이 익숙하지 않다면 test/java/lotto/LottoTest를 참고하여 학습한 후 테스트를 구현
