...
public class LottoReader {
    public static List<LottoRank> rank(List<Lotto> lottos, Lotto drawnLotto, int bonusNumber) {
        return lottos.stream()
                .map(lotto -> rank1Lotto(lotto, drawnLotto, bonusNumber))
                .collect(Collectors.toList());
    }

    private static LottoRank rank1Lotto(Lotto lotto, Lotto drawnLotto, int bonusNumber) {
    ...
    private static LottoRank judgeRank(int theNumberOfMatch, boolean isBonusNumberMatches) {
