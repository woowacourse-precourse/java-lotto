package lotto;

public class LottoConstant {
    public static final int MIN_LOTTO_NUM = 1;
    public static final int MAX_LOTTO_NUM = 45;
    public static final int LOTTO_SIZE = 6;
    public static final int LOTTO_PRICE = 1000;
    public static final int WINNING_THREE_NUM = 3;
    public static final int WINNING_FOUR_NUM = 4;
    public static final int WINNING_FIVE_NUM = 5;
    public static final int WINNING_SIX_NUM = 6;
    public static final int INIT_WINNING_COUNT = 0;
    public static final int PLUS_ONE_COUNT = 1;
    public static final int WINNING_THREE_MONEY = 5000;
    public static final int WINNING_FOUR_MONEY = 50000;
    public static final int WINNING_FIVE_MONEY = 1500000;
    public static final int WINNING_FIVE_AND_BONUS_MONEY = 30000000;
    public static final int WINNING_SIX_MONEY = 2000000000;
    public static final int ROUND_POINT = 2;

    public static final String PURCHASE_PRICE_MSG = "구매금액을 입력해 주세요.";
    public static final String WINNING_NUM_MSG = "당첨 번호를 입력해 주세요.";
    public static final String BONUS_NUM_MSG = "보너스 번호를 입력해 주세요.";
    public static final String BUY_MSG = "%d개를 구매했습니다.\n";
    public static final String WINNING_RESULT_MSG = "당첨 통계\n---\n총 수익률은 %.1f%%입니다.";
    public static final String SEPARATOR = ",";
}
