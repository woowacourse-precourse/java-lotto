...
> 부제: Enum 클래스의 static 변수와 메서드가 Enum 객체들 생성 이후에 돌아가는 이유
...
> Enum 클래스, 6개의 enum을 지정해놓았다. 생성자를 통해 생성 시 `updateSearchTool`을 통해 해당 객체값을 `matchPrize`에 등록한다. (메서드에 static이 붙어있는 건 다른 클래스에서 옮겨오다가 깜빡하고 지우지 않음..)
