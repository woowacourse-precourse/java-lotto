...
- [ ] 핵심 비즈니스 로직을 처리할 service
    - [ ] 구매 금액을 입력받아 lottery 생성
    - [ ] 보너스 번호와 당첨 번호를 입력받아 winningNumbers 생성
    - [ ] lottery 의 순위계산 메소드 호출

- [ ] 프로그램의 흐름을 관리하는 controller
    - [ ] 구매금액를 입력받아, LottoService 에 전달
    - [ ] 당첨번호와 보너스 번호를 입력받아, LottoService 에 전달
    - [ ] lottoService 의 순위 계산 메소드를 호출하고, OutputView의 결과출력 메소드 호출
    - [ ] 생성된 로또들의 정보를 String 으로 반환하는 메소드
