...
- 총 로또 당첨 금액을 구하는 기능
- 당첨 로또를 입력으로 받아 전체 [수익 정보](https://www.notion.so/3-6c12ced00f0a44ee836941407ea1a1f8)를 반환하는 기능
  - 전체 수익을 구하는 기능
  - 전체 수익률을 구하는 기능
  - 각 등수 별 당첨 횟수를 구하는 기능
</aside>
...
- 입력을 담당하는 모듈에서 정제된 사용자 입력을 받아옴
- 출력을 담당하는 모듈에 데이터를 전달 → 정해진 형식으로 출력 되도록 함
- **예외 발생 시**  입출력 매니저에 예외 객체를 전달, 에러 메시지가 출력 되도록 함
...
- 입력
  ...
  - 사용자로부터 **로또 숫자 목록**을 입력 받는 기능
    - 이때, 각 숫자를 `,`로 구분
  - 사용자로부터 **보너스 숫자**를 입력 받는 기능

  잘못된 입력이 주어진 경우 `IllegalArgumentException` 반환
...
총 수익 및  수익률, 등수 별 당첨 횟수 등을 담고 있는 객체

<aside>
🔔 **기능 목록**

- 등수를 입력하면, 당첨 횟수를 반환하는 기능
- 총 수익 / 수익률을 반환하는 기능
</aside>