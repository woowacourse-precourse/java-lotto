...
UI를 제외하고 단위 테스트 만들기
### Lotto 클래스
* [x] 로또 발행 횟수 테스트 `getPublishNum()`
* [x] 발행된 로또 오름차순 정렬 테스트 `sortNumbers()`
* [x] 발행된 로또의 수량과 로또 발행 횟수가 다를 때 예외 발생 테스트 `validateLotto()`
...
* [x] 로또 분석
   ...
   - [x] 로또 당첨 번호와 5개가 일치하고 보너스 번호와 일치할 때
   - [x] 로또 당첨 번호와 5개가 일치하고 보너스 번호와는 불일치할 때
   - [x] 로또 당첨 번호와 4개가 일치할 때
   - [x] 로또 당첨 번호와 3개가 일치할 때
   - [x] 하나도 당첨되지 않았을 때

### Input 클래스 : 유효성 검사
* [ ] 로또 구입 금액 입력 시 1000 단위의 숫자를 입력하지 않으면 예외 발생
* [ ] 로또 당첨 번호 입력 시 `1-45` 숫자와 `콤마(,)` 또는 공백만 허용 이 외 예외 발생
* [ ] 보너스 번호 입력 시 `1-45` 숫자만 허용 이 외 예외 발생