...
1. 로또 구입 금액을 입력 받는다. 
2. 구입 금액에 해당하는 만큼 로또가 발행된다.
3. 당첨 번호 6자리를 쉼표로 구분하여 입력한다.
4. 보너스 번호 한개를 입력 한다.
...
## 기능 목록 checkList
  ...
  - [x] 구입 금액에 해당하는 만큼 로또를 발행한다. - LottoMachine#createLottoNumber()
  ...
  - [x] 당첨 번호와 보너스 번호를 입력
    ...
    - [x] 7개의 수는 중복되지 않는다.
    - [x] 잘못된 번호 혹은 중복된 번호인 경우 `IllegalArgumentException`
...
## 테스트 목록
 ...
 - [x] 당첨 번호(로또 번호)
   - [x] 6자리여야 한다.
   - [x] 당첨 번호는 숫자만 입력 가능하다.
   - [x] 당첨 번호는 중복이 없어야 한다.
 
 - [x] 보너스 번호는 1개
   - [x] 보너스 번호는 숫자만 입력 가능하다.
 ...
 - [x] 총 상금 계산
   - [x] 당첨된 로또 수 만큼 랭킹에 따른 총 상금을 계산한다.
