...
![프로그램 주요 흐름 이미지](https://user-images.githubusercontent.com/70354365/201833422-b46742ab-a9df-439d-9e95-e7838fb84fe0.png)

1. 사용자가 `구매 금액`을 입력하면, 구매 금액에 해당하는만큼 `로또`를 구매합니다. (로또 구매 방식은 직접 번호를 정하는 것이 아닌 `자동 추첨`으로 각 로또를 발행합니다.)
1. 로또 1장 판매 금액이 `1,000`원 인 경우, 위 그림의 경우 총 `3`개의 로또를 구매할 수 있습니다.
1. 사용자가 `로또 당첨 번호`와 `보너스 번호`를 각각 입력하면, 이를 기반으로 당첨 정보를 저장합니다.
1. 사용자가 구매한 로또 리스트 (한 개 이상)와 당첨 정보를 비교하여 사용자의 각 로또의 당첨 여부를 확인합니다.
1. 통계 기록을 통해 사용자는 자신의 `당첨 내역`과 `수익률`을 확인할 수 있습니다. 
---
