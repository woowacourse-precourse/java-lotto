...
- [X] 당첨 금액 통계를 낸다. controller/model#myStat
...
- [X] 구입 금액은 1,000원 단위로 입력 받으며 1,000원으로 나누어 떨어지지 않는 경우, 그리고 음수를 받은경우 예외 처리한다.
- [X] 사용자가 입력하는 로또번호의 자릿수가 1~45가 맞는지, 음수는 아닌지 확인한다.
- [X] 사용자가 입력하는 모든 값에서 공백이 들어갈 경우 예외처리를 해야한다.
- [ ] 정답 로또번호와 보너스 번호가 중복되면 안된다. 
